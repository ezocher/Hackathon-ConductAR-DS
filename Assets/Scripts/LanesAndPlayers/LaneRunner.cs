...
public class LaneRunner : MonoBehaviour
{
    ...
    {
        float scaledDistance = distance * Time.deltaTime;
        float newZ = playerObject.transform.localPosition.z + scaledDistance;
        if ((newZ <= laneFarthestDistanceMeters) && (newZ >= laneClosestDistanceMeters))
        {
            Debug.Log("@ Player #" + laneNumber + ": Moving " + scaledDistance + " meters");
            playerObject.transform.Translate(0f, 0f, scaledDistance);
        }
